...
** To use this file, add lines similar to the following to ~/.emacs:
   #+BEGIN_SRC .emacs-file
     ...
     ;;load the correct org
     (add-to-list 'load-path (concat emacs-git "Plugins/org-mode-mirror"))
     (add-to-list 'load-path (concat emacs-git "Plugins/org-mode-mirror/lisp"))
     (add-to-list 'load-path (concat emacs-git "Plugins/org-mode-mirror/contrib/lisp"))
...
** Emacs main window size
   #+BEGIN_SRC emacs-lisp
     ...
     (setq my-height-fraction 0.93) ;use the entire screen height
...
* Indentations
  #+BEGIN_SRC emacs-lisp
    ...
    (defun my-indent-mode-cree ()
      "Set indent tabs to the cree indent mode"
      (interactive)
      ;; C++
      (setq my-indent 4)
    ...
    (defun my-indent-mode-nano ()
      "Set indent tabs to the nano indent mode"
      (interactive)
      ;; C++
      (setq my-indent 2)
      (setq my-substatement 2)
      ...
      (setq-default tab-width 8) ;tabs are sometimes used by other people

      ;; Python - override
      (setq python-indent-offset 4)
      )      

    (defun my-indent-mode-bruker ()
      "Set indent tabs to the bruker indent mode"
      (interactive)
      ;; C++
      (setq my-indent 4)
      (setq my-substatement 4)
      ...
      (setq-default tab-width 4) ;tabs are sometimes used by other people
...
*** Bullets
 ...
 ;    (load "org-bullets.el")
     ...
     (defun my-org-hook ()
       ...
       (setq org-catch-invisible-edits 'show-and-error) ;when trying to edit hidden paragraphs open them and show error
    ...
    #+BEGIN_SRC emacs-lisp
      ...
      (if (string-match "mingw-nt" system-configuration)
          (progn
            (setq org-file-apps
                  (append
                  '(("png" . "\"c:/Program Files/giv/bin/giv.exe\" %s"))
                  '(("doc" . "\"c:/Program Files/Microsoft Office/root/Office16/WINWORD.EXE\" %s"))
                  '(("svg" . "\"c:/Program Files/Microsoft Office/root/Office16/c:/Program Files/Inkscape/inskscape.exe\" %s"))
                   org-file-apps
                   ))
            )
        (progn
...
** Shell tab-completion
   ...
   #+END_SRC
* Encoding
** Prevent encoding issues when saving files
   This prevents errors like
   _"These default coding systems were tried to encode text in the buffer ‘ *temp*’:"_
   #+BEGIN_SRC emacs-lisp
      (prefer-coding-system 'utf-8)
    #+END_SRC